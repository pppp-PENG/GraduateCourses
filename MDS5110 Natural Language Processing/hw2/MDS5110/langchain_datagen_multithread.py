...
def langchain_datagen(args):
    ...
    with jsonlines.open(
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            # 使用tqdm显示进度
            for future in tqdm(futures, total=len(items_to_process), desc="处理项目中"):
                try:
                    writer.write(future.result())
                except Exception as e:
                    print(f"处理项目时出错: {futures[future]['query']}. 错误: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path", type=str, required=True, help="输入JSONL文件的路径。"
    )
    parser.add_argument(
        "--output_path", type=str, required=True, help="输出JSONL文件的路径。"
    )
    parser.add_argument(
        "--max_workers",
        type=int,
