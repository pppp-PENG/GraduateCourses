...
def plot_mesh_labels(
    plt, model, x_min, x_max, y_min, y_max, x_label, y_label, step=0.01
):
...
def plot_mesh_labels_3d(
    plt, model, x_min, x_max, y_min, y_max, x_label, y_label, step=0.01
):
    xx, yy = np.meshgrid(np.arange(x_min, x_max, step), np.arange(y_min, y_max, step))
    mesh_points = np.c_[xx.ravel(), yy.ravel()]
    # create the third feature (square of sepal width) for mesh points
    third = (mesh_points[:, 0] ** 2).reshape(-1, 1)
    mesh_3d = np.c_[mesh_points, third]
    mesh_labels = model.predict(mesh_3d)
    mesh_labels_array = mesh_labels.reshape(xx.shape)
...
Y3_err = Y[Y != Y3_pred]
print(
    "\nThere are {} errors/mismatches in Logistic Regression (3-D)".format(
        sum(Y != Y3_pred)
    )
)
...
plt.scatter(X[:, 0], X[:, 1], c=Y, facecolor="k", cmap=plt.cm.RdYlBu)
plt.scatter(X[100, 0], X[100, 1], edgecolor="k")  # highlight the outlier
plt.scatter(X3_err[:, 0], X3_err[:, 1], marker="x", c="k")  # mark the errors
plt.title(f"Decision boundary of a Logistic Regression (3-D) by {stu_id}")
plt.axis("tight")
plt.show()

svc3 = LinearSVC(max_iter=1000000, dual=True, random_state=0)
svc3.fit(X3, Y)
Ysvc_pred = svc3.predict(X3)
Xsvc_err = X[Y != Ysvc_pred]
print("\nThere are {} errors/mismatches in LinearSVC (3-D)".format(sum(Y != Ysvc_pred)))
...
plt.scatter(X[:, 0], X[:, 1], c=Y, facecolor="k", cmap=plt.cm.RdYlBu)
plt.scatter(X[100, 0], X[100, 1], edgecolor="k")  # highlight the outlier
plt.scatter(Xsvc_err[:, 0], Xsvc_err[:, 1], marker="x", c="k")  # mark the errors
plt.title(f"Decision boundary of a LinearSVC (3-D) by {stu_id}")
plt.axis("tight")
plt.show()
