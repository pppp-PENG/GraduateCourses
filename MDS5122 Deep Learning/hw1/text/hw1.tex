...
\begin{enumerate}
    ...
    \begin{enumerate}[label = (\alph*)]
        ...
        \begin{table}[H]
            ...
            \begin{tabular}{cccccc}
                \toprule
                \textbf{Param} & \textbf{Epoch} & \textbf{Speed} & \textbf{Loss} & \textbf{Train Acc} & \textbf{Test Acc} \\
                \midrule
                46.87M & 64 & 16s/epoch & 0.1004 & 99.00\% & 38.82\% \\
                \bottomrule
            \end{tabular}
        \end{table}
        We can see that the test accuracy is 38.82\%, but the loss is extrimely low (0.1004) and the training accuracy is very high (99.00\%).
        ...
        \item \textbf{More Data Augmentation}:
        Add strongly RandomAffine and ColorJitter, and make the model more deeper and wider to improve the performance. \\
        And to accelarate the training speed, I modify the batch size to 1000 and number of workers to 8 in dataloader.
        (Details of the model structure is in the code files.)
        \begin{table}[H]
            \caption{Preferance of Improved Model}
            \centering
            \begin{tabular}{cccccc}
                \toprule
                \textbf{Param} & \textbf{Epoch} & \textbf{Speed} & \textbf{Loss} & \textbf{Train Acc} & \textbf{Test Acc} \\
                \midrule
                17.50M & 256 & 19s/epoch & 0.9946 & 76.44\% & 46.40\% \\
                \bottomrule
            \end{tabular}
        \end{table}
        \begin{figure}[H]
            \centering
            \includegraphics[width=1\textwidth]{../trainingLoss.png}
    ...
    \begin{enumerate}[label = (\alph*)]
        ...
        --~Solution: I rewrite the dataset class (myDataset.py), putting the transform operations in the new dataset class instead of huggingface's default dataset class, which can accelarate the data loading speed significantly.
    \end{enumerate}
    \item \textbf{Codes and Model Structures} \\
    \textbf{hw1\_cifar10.ipynb}: The code of the best model in my experiments for cifar10 experiments. \\
    \textbf{hw1\_tiny\_imagenet.ipynb}: The code of the best model in my experiments for tiny-imagenet experiments. \\
    \textbf{myDataset.py}: The dataset class for tiny-imagenet dataset. \\
    \textbf{note4cifar10.md}: The model structures and their performance for cifar10 experiments. \\
    \textbf{note4tinyimagenet.md}: The model structures and their performance for tiny-imagenet experiments.
\end{enumerate}
